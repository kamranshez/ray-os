"""
YouTube Lab — Streamlit app for managing thumbnail A/B tests.
...
def get_output_dir(video_id, info):
    """Get the output directory for a video, using output_folder if set."""
    folder = info.get("output_folder", video_id)
    return OUTPUT_DIR / folder

def get_thumbnails(video_id, info=None):
    """Get all thumbnails for a video."""
    video_dir = get_output_dir(video_id, info) if info else OUTPUT_DIR / video_id
...
if page == "Dashboard":
    ...
    sorted_videos = sorted(videos.items(), key=lambda x: x[1].get("publish_date") or "9999-99-99")
...
# --- Review ---
    ...
    # Sort by publish date (oldest first)
    sorted_available = sorted(available.items(), key=lambda x: x[1].get("publish_date") or "9999-99-99")
    ...
    selected_output_dir = get_output_dir(selected_vid, selected_info)
    thumbnails = get_thumbnails(selected_vid, selected_info)

    if not thumbnails:
        folder = selected_info.get("output_folder", selected_vid)
        st.warning(f"No thumbnails found in `output/{folder}/`")
        st.stop()
    ...
    # Style source mapping — prefix → (label, reference image path)
    REF_DIR = APP_DIR.parent / "research" / "competitor-thumbnails"
    STYLE_REFS = {
        "folder-": ("Nate Herk — Folder /command", REF_DIR / "nateherk" / "OUyfxhFtGCo.jpg"),
        "icons-": ("Nate Herk — Icons on black", REF_DIR / "nateherk" / "LrgfmZkl3nc.jpg"),
        "basic-vs-pro-": ("Nate Herk — Basic vs Pro", REF_DIR / "nateherk" / "ZeJXI2MAhj0.jpg"),
        "old-vs-new-": ("Nate Herk — Old vs New", REF_DIR / "nateherk" / "pkSxISewcw8.jpg"),
        "feature-grid-": ("Nate Herk — Feature grid", REF_DIR / "nateherk" / "4Zaoo0YbYaw.jpg"),
        "whiteboard-": ("Nate Herk — Whiteboard list", REF_DIR / "nateherk" / "mpALXah_PBg.jpg"),
        "retro-": ("Nate Herk — Retro arcade", REF_DIR / "nateherk" / "l1jnOXc52NY.jpg"),
        "cli-": ("Nate Herk — CLI terminal", REF_DIR / "nateherk" / "vFepZE_wrfg.jpg"),
        "dashboard-": ("Nate Herk — Dashboard stats", REF_DIR / "nateherk" / "NDnv16PY2XQ.jpg"),
        "agent-network-": ("Nate Herk — Agent network", REF_DIR / "nateherk" / "vDVSGVpB2vc.jpg"),
        "network-": ("Nate Herk — Agent network", REF_DIR / "nateherk" / "vDVSGVpB2vc.jpg"),
        "prop-": ("Nate Herk — Holding prop", REF_DIR / "nateherk" / "T6_Ges4j1qY.jpg"),
        "flow-": ("Nate Herk — Flow diagram", REF_DIR / "nateherk" / "hem5D1uvy-w.jpg"),
        "cinematic-": ("Fireship — Dark cinematic", REF_DIR / "other" / "HIp8sFB2GGw.jpg"),
        "clean-": ("VS Code — Blue gradient", REF_DIR / "other" / "dutyOc_cAEU.jpg"),
        "terminal-": ("Net Ninja — Terminal split", REF_DIR / "other" / "SUysp3sJHbA.jpg"),
        "minimal-": ("Kevin Stratvert — Minimal bold", REF_DIR / "other" / "eMZmDH3T2bY.jpg"),
        "text-only-": ("Tech With Tim — Text only", REF_DIR / "other" / "ntDIxaeo3Wg.jpg"),
        "bright-": ("Nathan Sebhastian — Bright comparison", REF_DIR / "other" / "7AImkA96mE8.jpg"),
        "tier-list-": ("DevOps Toolbox — Tier list", REF_DIR / "other" / "e9j2iEwJru0.jpg"),
    }

    def get_style_ref(filename):
        for prefix, (label, ref_path) in STYLE_REFS.items():
            if filename.startswith(prefix):
                return label, ref_path
        return "", None
    ...
    for variant_name, variant_files in variants.items():
        cols = st.columns([1] + [1.5] * len(variant_files))

        # Reference thumbnail + label
        label, ref_path = get_style_ref(variant_name)
        with cols[0]:
            if ref_path and ref_path.exists():
                st.image(str(ref_path), use_container_width=True)
            st.caption(label if label else variant_name)

        for j, fname in enumerate(variant_files):
            with cols[j + 1]:
                img_path = selected_output_dir / fname
            ...
            with st.expander(f"🔵 {info['video_number']} — {info['title'][:50]} · Due: {due} ({days_left})"):
                ...
                if uploaded_thumbs:
                    ...
                    for j, fname in enumerate(uploaded_thumbs):
                        with cols[j]:
                            img_path = get_output_dir(vid, info) / fname
                            if img_path.exists():
                                st.image(str(img_path), use_container_width=True)
                            st.caption(fname)
