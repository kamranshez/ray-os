...
description: Write hook-led teaser newsletters in the Matt Pocock / AI Hero mould — short emails whose job is to make the reader click a single link to a longer piece. Built on a hook + mechanism-with-numbers + bullet preview + one CTA arc. Use when Ray wants a punchy email-to-article teaser, a "write it like Pocock" newsletter, or a tight email driving traffic to a video or post. Triggers on "write a teaser newsletter", "Pocock-style email", "newsletter writer teaser", "write this like Matt Pocock", "AI Hero style email", or any request for a short opinionated newsletter that ends in one CTA.
---

# Newsletter Writer, Teaser / AI Hero Style
...
This skill is modelled on 10 real Matt Pocock (AI Hero) emails stored in `references/pocock-emails/`. Read them before writing. They are short. Most teasers are 150 to 250 words. Aim for that, not Ali Abdaal length.
...
Matt Pocock himself uses spaced hyphens for asides (" - context, vibe, and all - "). Do **not** copy that habit. Convert any pause-that-wants-a-dash into either a comma, a period, or a parenthetical.
...
- **American spelling.** Even though Pocock is British, his newsletter writes American (optimize, behavior). Match that.
- **Curly quotes feel natural.** Pocock's emails use `'` and `"` not `'` and `"`. Default to curly when writing prose-style body content.

### The Teaser Arc (the default shape, 150 to 250 words)

1. **Greeting**, `Hey [name],` then a blank line. That's it. Don't add a "hope you're well". (Pocock's emails frequently ship with a broken merge tag and just say `Hey ,` — fine to ignore, but evidence of how little ceremony there is.)
...
7. **Sign-off**, `Matt` or `-Matt`. No "Talk soon". No "Have a great week". Adapt to the actual writer (for Ray, use `Ray` or `-Ray`).
...
- **No long meandering stories.** If you find yourself writing a 4-paragraph personal anecdote, you're in Ali Abdaal mode. Cut it back to one line of personal credibility.
...
Pocock writes in four distinct shapes. Pick one before drafting. The differences are mostly **link count, bullet presence, and length**.

**A. Teaser-with-bullets (the default, ~150–250 words).** Hook → mechanism → 3-5 bullet preview → one CTA link → sign-off. This is the workhorse and the right answer ~70% of the time. Examples: emails 02, 03, 07, 08.

**B. Announcement / changelog (~150–250 words, no bullets).** Used when launching a new feature/skill/changelog. Hook is lighter (less contrarian, more "here's what shipped"), body is prose explaining each thing in one short paragraph, ends with one CTA link (often `Watch the video →`). No bullet preview, because the body itself is the preview. Examples: emails 04, 05.

**C. Long-form essay / sales letter (~500–900 words).** Used at cohort-launch moments. Opens with a setup-then-reveal personal moment ("I was expecting to sell ~800. We sold 2,500."). Uses ASCII dividers (`-----`) as inline section breaks. Quotes students/users in pulled-out blocks. Explicit pricing + deadline. P.S. at the bottom. Same direct claim-first voice, just more of it. Examples: emails 06, 10. **Only use this when Ray explicitly wants a longer reflection piece.**

**D. Micro single-question email (~40 words, no link).** A direct question to the reader to elicit a reply. No bullets, no CTA, no mechanism paragraph. Just `Hey [name], <one-sentence setup>. <direct question>. -Matt`. Example: email 09.
...
- **No "Now I know what some of you are probably thinking..."** That's Ali Abdaal. Pocock doesn't pre-empt objections, he just makes the claim and moves on.
- **No "course"/"class" framing for Ray's content.** Always refer to the Agentic Coding School as a "masterclass".
...
Pocock emails almost never end on a "reply and let me know" question, they end on a CTA link. If you want a reply prompt (Ray sometimes does), keep it brutally short, one line, and place it just before the sign-off. Apply the same framing logic as before:
...
The typical input is a video transcript that needs to be turned into a short, Pocock-style email. The output is 5 body variations plus a global pool of 10 subject lines and 10 preview snippets. Ray picks one body, one subject, and one preview, and copies the trio.
...
**10 Subject Lines** (global pool). Pocock subjects are short, declarative, often quotable. Examples from the corpus:
...
**10 Preview Snippets** (global pool). One-sentence teases that complement the subjects without repeating them. Pocock previews tend to lead with the twist or the credibility ("After months of coding with AI...", "It's an official tool that's fundamentally broken...", "We sold 2,500.").
...
In markdown: write the 5 bodies first, then a "Subject line options" list of 10, then a "Preview snippet options" list of 10, then the CTA questions (if any). In the HTML viewer: bodies, subjects, and previews flow into the template (see the HTML Viewer section).
...
Some newsletters include bonus content below the sign-off (system prompts, code snippets, checklists). Pocock-style emails rarely do this, but if the content lends itself:
...
4. Draft 5 complete newsletter variations. **Target 150 to 350 words each by default.** If you find yourself writing 600+ words, cut.
...
9. **Dash scan.** Literally search every variation for `—` and `–` and remove them. Replace with commas, periods, parentheses, or a rewrite. Pocock himself uses spaced hyphens (` - `) for asides, convert those too. Do this even if you "know" you didn't add any.
...
10. **Handle the thumbnail** (video-redirect newsletters only). Ask Ray for a thumbnail path if not provided. Run `scripts/add-play-button.py`.
11. Present everything together for Ray to mix and match. If Ray asks for HTML/viewer mode, render via `references/gmail-viewer-template.html`.
12. After the email is sent, ask Ray to paste the final version back so it can be stored in `references/sent/` for future reference and performance tracking.

## Examples

See `references/pocock-emails/` for 10 real Matt Pocock newsletters. Read these to calibrate. `INDEX.md` lists them all.

**Best examples to study (in priority order):**
1. `03-my-skill-makes-claude-great-at-tdd.md`, textbook Pocock Arc, short, claim + setup + bullet list + CTA
...
Ignore `01-...confirm-your-subscription...` (transactional, not a newsletter). Ignore `05-...ubiquitous-language-is-dead...` for hooks (it's a quiet changelog update, not a hook-led email).
