export const meta = {
  name: 'idea-foundry',
  description: 'Mine 60-150 fresh ideas on a topic via ~20 parallel agents (Exa web search + X search via MCP + pure reasoning), synthesize, render to a self-contained HTML idea bank with selectable cards and a floating copy button.',
...
const xThunks = X_QUERIES.map(a => () => agent(
  `${CONTEXT}\n\nYou are an idea-generation agent for the X-search angle: "${a.key}" (${a.desc}).\n\nSearch X via the rapidapi-twitter MCP. Load the tool first:\n\n  ToolSearch with query: select:mcp__rapidapi-twitter__Search\n\nThen call mcp__rapidapi-twitter__Search with:\n  query: '${a.query} min_faves:20 -filter:replies since:${RECENT_SINCE}'\n  search_type: 'Top'\n\nThe query string is forwarded to X verbatim, so its advanced-search operators (from:, min_faves:, lang:, since:, -filter:replies, url:, OR) all work. Run 2 to 4 calls with varied operator combinations to triangulate rather than one broad call. Paginate by passing the response's next_cursor back as 'cursor' — 2 to 3 pages per query is plenty. Dedupe by tweet_id across calls.\n\nRank what comes back by engagement relative to author reach (a 10k-follower account with 500 likes is sharper signal than a 10M-follower account with 5k). Ignore results that are all tiny accounts with near-zero engagement, and ignore the endpoint's own promotional/rate-limit notes.\n\nIf the tool errors, returns no timeline, or every call comes back empty, return ideas: [].\n\nOtherwise extract 6 to 10 FRESH ideas informed by what people on X are actually saying. The ideas should be triggered by the tweets but go beyond them (a tweet is a seed, not the whole idea).\n\nEach idea: title (4-10 words, no dashes), 3-5 sentence explanation (NO em or en dashes), level, category, source_urls=[the tweet URLs that inspired this idea, built as https://x.com/<screen_name>/status/<tweet_id>], angle="${a.key}".\n\nReturn via the schema.`,
