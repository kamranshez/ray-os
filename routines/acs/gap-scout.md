...
(d) PROMOTE THE SPINE IDEAS — from the extracted IDEAS and INSIGHTS, pick the 1-2 (max 3) that are load-bearing: the idea that, if you understood only it, would let you reconstruct or subsume most of the others. This is NOT the most frequent idea or the catchiest tip — it is the one with the most altitude, the reframe the rest hang off. Most videos have exactly one. A pure news/announcement video may have zero spine ideas worth a video — if so, return covered, log it, and post nothing.
  PROCEDURE: (1) Cluster your IDEAS/INSIGHTS into 2-4 themes; a single theme may yield MORE than one spine. (2) For each candidate spine, write the one sentence, then list which other IDEAS it explains or makes predictable. (2b) DE-MERGE TEST — before letting any spine absorb an idea sitting in its cluster, ask of that absorbed idea: would it become a DIFFERENT video — distinct central DEMO, distinct ACS class/chapter SLOT, and distinct "one thing you can do after"? If it clears at least the DEMO and the SLOT bars, PROMOTE IT AS ITS OWN SPINE even though the broader idea nominally subsumes it. The strategy idea explains *why*; the nested technique is *what you actually film* — film-able techniques do not ride along as sub-beats. (3) A candidate qualifies as a spine by EITHER of two independent routes — do not let one veto the other:
     ROUTE A (breadth): it accounts for the MOST other bullets — explains ≥~40% of them. "Explains" means it makes the other bullets PREDICTABLE, not merely that it is a broader topic those bullets fall under — a strategy that topically contains a concrete technique has NOT subsumed it (see 2b); count such a technique as its own candidate, not as breadth for the umbrella. A candidate that explains fewer than ~40% AND offers no standalone reframe is a tip, not a spine.
  ...
  EXAMPLE (Route B): a single exchange noting "agents laser-focus on the one task and fix the instance, not the class" is a LATENT SPINE — its own buildable video about local fixes vs architectural/global fixes — even though it recurs nowhere else in the transcript. Do NOT demote it to a sub-beat just because it is brief. COROLLARY (the inverse failure): do NOT demote a concrete, demo-able technique into an adjacent higher-altitude strategy spine just because the strategy is catchier or "contains" it — a technique with its own demo and its own catalog slot is a spine in its own right, however often it recurs.
...
(f) GAP + RELATE vs the ACS catalog — for EACH spine idea (and spot-check the broader IDEAS list), call Agentic-Coding-School `search_videos` with a targeted query. For each spine idea, find the SPECIFIC existing ACS video(s) it touches and classify the RELATIONSHIP, not just coverage. FEEDBACK: if, while gap-checking, a sub-idea you folded into a spine would land in a DIFFERENT class/chapter slot than its parent spine, treat that as proof it is a separate spine — return to (d), promote it, and deep-dive it too.
