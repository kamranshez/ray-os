...
> 🎨 DRAW `decision-surfaces-hook-fable-leaving`: the model leaving on `[DATE]`, and the one thing it was worth using for on the way out — not better prompts, richer decisions

![[decision-surfaces-hook-fable-leaving-1.png]]
![[decision-surfaces-hook-fable-leaving-2.png]]
...
![[decision-surfaces-markdown-wall-hidden-flaw-3.png]]
![[decision-surfaces-markdown-wall-hidden-flaw-4.png]]
![[decision-surfaces-markdown-wall-hidden-flaw-5.png]]
...
> 🎨 DRAW `decision-surfaces-victor-cheap-representations`: the old world where a custom representation costs days of hand-work so everything collapses down the cheap-text path, versus the new world where it costs minutes; the expensive gate is broken open and only habit still steers you to text

![[decision-surfaces-victor-cheap-representations-1.png]]
![[decision-surfaces-victor-cheap-representations-2.png]]
...
> 🎨 DRAW `decision-surfaces-onboarding-dead-end`: the fresh user walked straight into a Test Recording step that cannot succeed (canRecord false, license never asked for), beside the fix plan forking into two tinted lanes that branch and rejoin

![[decision-surfaces-onboarding-dead-end-1.png]]
![[decision-surfaces-onboarding-dead-end-2.png]]
...
![[decision-surfaces-sixteen-representations-grid-1.png]]
![[decision-surfaces-sixteen-representations-grid-2.png]]
...
> 🎨 DRAW `decision-surfaces-demo-prompt-canvas`: one typed prompt expanding into the parts of the canvas it asks for — top band of phone mockups, before/after diagrams beneath each stage, tinted branch lanes with fork and rejoin wires, stickies pinned next to stages, a Copy JSON button

![[decision-surfaces-demo-prompt-canvas-1.png]]
![[decision-surfaces-demo-prompt-canvas-2.png]]
...
![[decision-surfaces-playfair-table-to-chart-1.png]]
![[decision-surfaces-playfair-table-to-chart-2.png]]
...
> 🎨 DRAW `decision-surfaces-walk-the-board`: the canvas seen from above like a general's map table — top band of phone screens, before/after diagrams beneath, a tinted lane forking and rejoining, purple decision stickies and yellow open questions pinned beside the stages they affect

![[decision-surfaces-walk-the-board-1.png]]
![[decision-surfaces-walk-the-board-2.png]]
...
![[decision-surfaces-canvas-review-loop-1.png]]
![[decision-surfaces-canvas-review-loop-2.png]]
...
> 🎨 DRAW `decision-surfaces-plan-mutates-v6-v7`: v6 and v7 side by side, the lower lane in v7 physically grown a new stage ("Create your first mode") slotted where comment three pointed, buy button gone, banner saying comments 1-4 applied

![[decision-surfaces-plan-mutates-v6-v7-1.png]]
![[decision-surfaces-plan-mutates-v6-v7-2.png]]
...
> 🎨 DRAW `decision-surfaces-honest-limits-three`: the two-file change that doesn't need a map table, beside a canvas that is still a picture not a model — a decision toggle that *should* rewire the lanes into a different world, drawn ghosted because it can't yet

![[decision-surfaces-honest-limits-three-1.png]]
![[decision-surfaces-honest-limits-three-2.png]]
...
![[decision-surfaces-everything-is-a-surface-1.png]]
![[decision-surfaces-everything-is-a-surface-2.png]]
...
> 🎨 DRAW `decision-surfaces-key-insight-quote`: one identical body of information pouring into two surfaces and producing two different sets of visible decisions — nothing added, only the surface changed

![[decision-surfaces-key-insight-quote-1.png]]
![[decision-surfaces-key-insight-quote-2.png]]
...
> 🎨 DRAW `decision-surfaces-close-choose-the-surface`: the fork where the same plan gets reviewed on a wall of text (choosing to miss things) or a rich spatial map (higher quality decisions) — representations now cost minutes, so the surface is a choice

![[decision-surfaces-close-choose-the-surface-1.png]]
![[decision-surfaces-close-choose-the-surface-2.png]]
