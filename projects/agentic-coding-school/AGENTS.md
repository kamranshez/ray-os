...
- `research/` — supporting material, not class scripts. (Note: this older `research/` is generic; audience/market research now lives under `context-layer/audience/`.)
- `context-layer/` — **the hub for who buys, how we position, and the rules every output must follow.** Read this before writing or revising landing copy, VSL scripts, ads, emails, or pricing. `context-layer/README.md` indexes it. Key nodes: `positioning/brief.md` (the single source of truth for the "Agent Engineer" reposition: villain, "engineer loops" lead, identity, audience, offer, team, proof, always-current), `positioning/open-decisions.md` (unconfirmed items), `audience/` (the 6-source market analysis; `audience/00-personas.md` is the synthesis, append don't redo), `pricing.md` (numbers, tiers, refund terms, policies), `promotion.md` (channels/launch), `voice.md` (tone + copy rules: no em/en dashes, locked lines, honesty guardrails).
- `vsl/` — VSL scripts (an output; consume `context-layer/`).

(Landing-page outputs will be regenerated from `context-layer/positioning/brief.md` when ready; the old `landing/` drafts were removed 2026-06-27. Implementation path: hard-swap the live `agent-engineer.tsx` with a patient, volume-based revert rule.)
